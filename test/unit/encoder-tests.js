...
describe("encoder", function () {
    ...
    describe("#encode() and #decode()", function () {
        ...
        it("should encode stringified uuids for backward-compatibility", function () {
            ...
            let encoded = typeEncoder.encode(
                uuid.toString(),
                types.dataTypes.uuid,
            );
            assert.strictEqual(
                encoded.toString("hex"),
                uuid.getBuffer().toString("hex"),
            );
        ...
        it("should throw when the typeInfo and the value source type does not match", function () {
            ...
            assert.throws(function () {
                typeEncoder.encode(
                    "Its anybody in there? I know that you can hear me",
                    dataTypes.blob,
                );
            }, TypeError);
            assert.throws(function () {
        ...
        it("should encode Long/Date/Number/String as timestamps", function () {
            ...
            let buffer = encoder.encode(
                types.Long.fromBits(0x00fafafa, 0x07090909),
                dataTypes.timestamp,
            );
            ...
            buffer = encoder.encode(
                new Date(1421755130012),
                dataTypes.timestamp,
            );
            assert.throws(function () {
                encoder.encode(
                    new Date("This is an invalid date string"),
                    dataTypes.timestamp,
                );
            }, TypeError);
            assert.strictEqual(buffer.toString("hex"), "0000014b0735a09c");
            ...
            assert.strictEqual(buffer.toString("hex"), "0000014b0735a09c");
            buffer = encoder.encode(
                "Tue Jan 20 2015 13:00:35 GMT+0100 (CET)",
                dataTypes.timestamp,
            );
            assert.strictEqual(buffer.toString("hex"), "0000014b07373ab8");
            assert.throws(function () {
                encoder.encode(
                    "This is an invalid date string",
                    dataTypes.timestamp,
                );
            }, TypeError);
        });
        ...
        it("should encode String/Long/Number as bigint", function () {
            const encoder = new Encoder(2, {});
            let buffer = encoder.encode(
                types.Long.fromString("506946367331695353"),
                dataTypes.bigint,
            );
        ...
        it("should encode String/Integer/Number as varint", function () {
            const encoder = new Encoder(2, {});
            let buffer = encoder.encode(
                types.Integer.fromString("33554433"),
                dataTypes.varint,
            );
        ...
        it("should encode String/BigDecimal/Number as decimal", function () {
            const encoder = new Encoder(2, {});
            let buffer = encoder.encode(
                types.BigDecimal.fromString("0.00256"),
                dataTypes.decimal,
            );
        ...
        it("should encode/decode InetAddress/Buffer as inet", function () {
            ...
            const encoder = new Encoder(2, {});
            let val1 = new InetAddress(
                utils.allocBufferFromArray([15, 15, 15, 1]),
            );
            ...
            val1 = new InetAddress(
                utils.allocBufferFromString(
                    "00000000000100112233445500aa00bb",
                    "hex",
                ),
            );
            ...
            assert.strictEqual(
                encoded.toString("hex"),
                val1.getBuffer().toString("hex"),
            );
        });

        it("should decode uuids into Uuid", function () {
            const uuid = types.Uuid.random();
            const decoded = typeEncoder.decode(uuid.getBuffer(), {
                code: dataTypes.uuid,
            });
        ...
        [2, 3].forEach(function (version) {
            ...
            it(
                util.format(
                    "should encode and decode maps for protocol v%d",
                    version,
                ),
                function () {
                    ...
                    encoded = encoder.encode(value, {
                        code: dataTypes.map,
                        info: [dataTypes.string, dataTypes.int],
                    });
                    decoded = encoder.decode(encoded, {
                        code: dataTypes.map,
                        info: [
            ...
            it(
                util.format(
                    "should encode and decode maps with stringified keys for protocol v%d",
                    version,
                ),
                function () {
                    let value = {};
                    ...
                    value = {};
                    value[types.Long.fromBits(0x12002001, 0x7f999299)] =
                        "bigint1";
                    value[types.Long.fromBits(0x12002000, 0x7f999299)] =
                        "bigint2";
            ...
            it(
                util.format(
                    "should encode and decode list<int> for protocol v%d",
                    version,
                ),
                function () {
                    const value = [1, 2, 3, 4];
            ...
            it(
                util.format(
                    "should encode and decode list<double> for protocol v%d",
                    version,
                ),
                function () {
            ...
            it(
                util.format(
                    "should encode and decode list<double> without hint for protocol v%d",
                    version,
                ),
                function () {
            ...
            it(
                util.format(
                    "should encode and decode set<text> for protocol v%d",
                    version,
                ),
                function () {
            ...
            it(
                util.format(
                    "should encode and decode list<float> with typeInfo for protocol v%d",
                    version,
                ),
                function () {
                    const value = [
                        1.1122000217437744, 2.212209939956665,
                        3.3999900817871094, 4.412120819091797, -1000, 1,
                    ];
                    const encoded = encoder.encode(value, {
                        code: dataTypes.list,
            ...
            it(
                util.format(
                    "should encode/decode ES6 Set as maps for protocol v%d",
                    version,
                ),
                function () {
                    ...
                    if (version === 2) {
                        assert.strictEqual(
                            encoded.toString("hex"),
                            "00030004000000010004000000020004000003e8",
                        );
                    }
                    decoded = encoder.decode(encoded, {
                        code: dataTypes.set,
            ...
            it(
                util.format(
                    "should encode/decode Map polyfills as maps for protocol v%d",
                    version,
                ),
                function () {
                    const encoder = new Encoder(version, {
                    ...
                    if (version === 2) {
                        assert.strictEqual(
                            encoded.toString("hex"),
                            "000200026b310002763100026b3200027632",
                        );
                    }
                    let decoded = encoder.decode(encoded, {
                        code: dataTypes.map,
                    ...
                    if (version === 2) {
                        assert.strictEqual(
                            encoded.toString("hex"),
                            "000300026b3100040000000100026b3200040000000200026b33000400000003",
                        );
                    }
                    decoded = encoder.decode(encoded, {
                        code: dataTypes.map,
            ...
            it(
                util.format(
                    "should encode/decode ES6 Map as maps for protocol v%d",
                    version,
                ),
                function () {
            ...
            it(
                util.format(
                    "should encode/decode Set polyfills as maps for protocol v%d",
                    version,
                ),
                function () {
                    const encoder = new Encoder(version, {
                        encoding: { set: helper.Set },
                    });
        ...
        it("should encode/decode nested collections", function () {
            ...
            const encoded = encoder.encode(
                { key1: ["first", "second", "third"], key2: ["2-first"] },
                type,
            );
            const decoded = encoder.decode(encoded, type);
        ...
        it("should encode/decode tuples", function () {
            ...
            const type = {
                ...
                info: [{ code: dataTypes.text }, { code: dataTypes.timestamp }],
            };
            const encoded = encoder.encode(
                new types.Tuple("one", new Date(1429259123607)),
                type,
            );
            const decoded = encoder.decode(encoded, type);
        ...
        it("should encode/decode LocalDate as date", function () {
            ...
            const dates = [
                // At epoch.
                {
                    ldate: new types.LocalDate(1970, 1, 1),
                    string: "1970-01-01",
                    date: new Date(Date.UTC(1970, 0, 1)),
                },
                // 10 days after epoch.
                {
                    ldate: new types.LocalDate(1970, 1, 11),
                    string: "1970-01-11",
                    date: new Date(Date.UTC(1970, 0, 11)),
                },
                // -10 days from epoch.
                {
                    ldate: new types.LocalDate(1969, 12, 22),
                    string: "1969-12-22",
                    date: new Date(Date.UTC(1969, 11, 22)),
                },
                // Year after 0.
                {
                    ldate: new types.LocalDate(1, 1, 1),
                    string: "0001-01-01",
                    date: year1day1,
                },
                // 0th year.
                {
                    ldate: new types.LocalDate(0, 1, 1),
                    string: "0000-01-01",
                    date: year0day1,
                },
                // Year before 0.
                ...
                {
                    ldate: new types.LocalDate(-271821, 4, 20),
                    string: "-271821-04-20",
                    date: new Date(Date.UTC(-271821, 3, 20)),
                },
                // Maximum possible ES5 date.
                {
                    ldate: new types.LocalDate(275760, 9, 13),
                    string: "275760-09-13",
                    date: new Date(Date.UTC(275760, 8, 13)),
                },
                // Minimum possible C* date.
                {
                    ldate: new types.LocalDate(-2147483648),
                    string: "-2147483648",
                    date: new Date(NaN),
                },
                // Maximum possible C* date.
                {
                    ldate: new types.LocalDate(2147483647),
                    string: "2147483647",
                    date: new Date(NaN),
                },
            ];
            ...
            dates.forEach(function (item) {
                ...
                assert.strictEqual(
                    decoded.toString(),
                    item.string,
                    "String mismatch for " + item.date,
                );
                if (isNaN(item.date.getTime())) {
                    assert.ok(isNaN(decoded.date.getTime()));
                } else {
                    assert.equal(
                        decoded.date.getTime(),
                        item.date.getTime(),
                        decoded.date + " != " + item.date,
                    );
                }
            });
        });
        ...
        it("should refuse to encode invalid values as LocalDate.", function () {
            ...
            assert.throws(function () {
                encoder.encode(23.0, type);
            }, TypeError);
            assert.throws(function () {
                encoder.encode("zzz", type);
            }, TypeError);
            assert.throws(function () {
                encoder.encode("", type);
            }, TypeError);
        ...
        it("should encode/decode LocalTime as time", function () {
            ...
            [
                ...
                const encoded = encoder.encode(
                    new types.LocalTime(types.Long.fromString(item[0])),
                    type,
                );
                const decoded = encoder.decode(encoded, type);
    ...
    describe("#encode()", function () {
            ...
            [2, 3].forEach((v) => {
                ...
                Array.from(zeroLengthTypesSupported).forEach(function (t) {
                    const type = {
                        ...
                        info: [{ code: types.dataTypes.text }, { code: t }],
                    };
        ...
        it("should decode 0-length map values of unsupported types into null values", function () {
            ...
            [2, 3].forEach((v) => {
                ...
                utils
                    .objectValues(types.dataTypes)
                    .filter(
                        (t) =>
                            typeof t === "number" &&
                            !zeroLengthTypesSupported.has(t),
                    )
                    .forEach((t) => {
                        const type = {
                            code: types.dataTypes.map,
        ...
        it("should decode empty buffers as nulls for most types", () => {
            ...
            utils
                ...
                .forEach((code) =>
                    assert.strictEqual(
                        encoder.decode(emptyBuffer, { code }),
                        null,
                    ),
                );
        });
        ...
        it("should decode empty buffers as empty strings", () => {
            ...
            [dataTypes.text, dataTypes.ascii, dataTypes.varchar].forEach(
                (code) =>
                    assert.strictEqual(
                        encoder.decode(emptyBuffer, { code }),
                        "",
        ...
        it("should decode empty buffers for blobs and custom types", () => {
            const encoder = new Encoder(4, {});
            const emptyBuffer = utils.allocBufferUnsafe(0);
            [dataTypes.blob, dataTypes.custom].forEach((t) =>
                assert.deepStrictEqual(
                    encoder.decode(emptyBuffer, { code: t }),
        ...
        it("should decode null map values", function () {
            ...
            const encoder = new Encoder(3, {});
            const buffer = utils.allocBufferFromString(
                "00000001000000046b657931FFFFFFFF",
                "hex",
            );
        ...
        it("should return the raw buffer when a buffer is provided", () => {
            ...
            Object.keys(dataTypes).forEach((typeName) => {
                assert.strictEqual(
                    encoder.encode(buffer, dataTypes[typeName]),
                    buffer,
                );
            });
        });
        ...
        it("should encode string representations of numeric values", () => {
            ...
            assertBuffer(
                encoder.encode("3.375", getType(dataTypes.decimal)),
                6,
            );
        ...
        it("should throw an error for invalid string representations of numeric values", () => {
            const encoder = newInstance();
            assert.throws(
                () => encoder.encode("hello!", getType(dataTypes.int)),
                TypeError,
            );
            assert.throws(
                () => encoder.encode("hello!", getType(dataTypes.double)),
                TypeError,
            );
            assert.throws(
                () => encoder.encode("hello!", getType(dataTypes.float)),
                TypeError,
            );
        });
    });
        ...
        it("should concat Array of buffers in the correct format", function () {
            let options = getExecOptions({
                ...
                routingKey: [
                    ...
                    utils.allocBufferFromArray([2]),
                    utils.allocBufferFromArray([3, 3]),
                ],
            });
    ...
    describe("#parseFqTypeName()", function () {
        it("should parse single type names", function () {
            const encoder = new Encoder(2, {});
            let type = encoder.parseFqTypeName(
                "org.apache.cassandra.db.marshal.Int32Type",
            );
            assert.strictEqual(dataTypes.int, type.code);
            type = encoder.parseFqTypeName(
        ...
        it("should parse complex type names", function () {
            ...
            type = encoder.parseFqTypeName(
                "org.apache.cassandra.db.marshal.SetType(org.apache.cassandra.db.marshal.TimeUUIDType)",
            );
            assert.strictEqual(dataTypes.set, type.code);
            assert.ok(type.info);
            ...
            type = encoder.parseFqTypeName(
                "org.apache.cassandra.db.marshal.VectorType(org.apache.cassandra.db.marshal.FloatType,10)",
            );
        ...
        it("should parse frozen types", function () {
            const encoder = new Encoder(2, {});
            let type = encoder.parseFqTypeName(
                "org.apache.cassandra.db.marshal.FrozenType(org.apache.cassandra.db.marshal.ListType(org.apache.cassandra.db.marshal.TimeUUIDType))",
            );
            assert.strictEqual(dataTypes.list, type.code);
            assert.ok(type.info);
            assert.strictEqual(dataTypes.timeuuid, type.info.code);
            ...
            type = encoder.parseFqTypeName(
                "org.apache.cassandra.db.marshal.MapType(org.apache.cassandra.db.marshal.UTF8Type,org.apache.cassandra.db.marshal.FrozenType(org.apache.cassandra.db.marshal.ListType(org.apache.cassandra.db.marshal.Int32Type)))",
            );
            assert.strictEqual(dataTypes.map, type.code);
            assert.ok(Array.isArray(type.info));
            assert.strictEqual(dataTypes.varchar, type.info[0].code);
        ...
        it("should parse udt types", function () {
            ...
            assert.strictEqual(
                dataTypes.varchar,
                dataType.info.fields[0].type.code,
            );
            assert.strictEqual("number", dataType.info.fields[1].name);
            assert.strictEqual(
                dataTypes.varchar,
                dataType.info.fields[1].type.code,
            );
        });
    ...
    describe("#parseTypeName()", function () {
        ...
        it("should parse complex type names", async () => {
            ...
            const items = [
                ...
                [
                    "map<varchar,bigint>",
                    dataTypes.map,
                    [dataTypes.varchar, dataTypes.bigint],
                ],
                [
                    "tuple<varchar,int>",
                    dataTypes.tuple,
                    [dataTypes.varchar, dataTypes.int],
                ],
                ["frozen<list<timeuuid>>", dataTypes.list, dataTypes.timeuuid],
                [
                    "map<text,frozen<list<int>>>",
                    dataTypes.map,
                    [dataTypes.text, dataTypes.list],
                ],
                [
                    "vector<float,20>",
                    dataTypes.custom,
        ...
        it("should parse nested subtypes", async () => {
            const encoder = new Encoder(4, {});
            const name =
                "map<text,frozen<list<frozen<map<text,frozen<list<int>>>>>>>";
        ...
        [true, false].forEach((frozen) => {
            it(`should parse udts with frozen ${frozen}`, async () => {
                const encoder = new Encoder(4, {});
                const udtResolver = sinon.spy(() =>
                    Promise.resolve("udt info dummy"),
                );
                ...
                assert.deepStrictEqual(udtResolver.firstCall.args, [
                    "ks_sample",
                    "address",
                ]);
            });
        ...
        it("should callback with TypeError when not found", async () => {
            ...
            await helper.assertThrowsAsync(
                encoder.parseTypeName("ks2", "WHATEVER", 0, null, udtResolver),
                TypeError,
            );

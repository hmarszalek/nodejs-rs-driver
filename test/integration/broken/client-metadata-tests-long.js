        ...
        before(function (done) {
            ...
            utils.series(
                [
                    ...
                    function (next) {
                        client.execute(
                            util.format(createQuery, "sampleks1", 2, 2),
                            next,
                        );
                    },
                    function (next) {
                        client.execute(
                            util.format(createQuery, "sampleks2", 3, 1),
                            next,
                        );
                    },
                ],
        ...
        it("should get the replica", function (done) {
            ...
            client.connect(function (err) {
                ...
                for (let i = 0; i < client.metadata.ring.length; i++) {
                    ...
                    const replica =
                        client.metadata.primaryReplicas[
                            client.metadata.tokenizer.stringify(token)
                        ];
    ...
    describe("#getReplicas() with RandomPartitioner", function () {
        ...
        it("should get the replica", function (done) {
            ...
            client.connect(function (err) {
                ...
                for (let i = 0; i < client.metadata.ring.length; i++) {
                    const token = client.metadata.ring[i];
                    const position = utils.binarySearch(
                        client.metadata.ring,
                        token,
                        (t1, t2) => t1.compare(t2),
                    );
                ...
                client.execute(
                    ...
                    [],
                    { routingKey: utils.allocBufferUnsafe(2) },
                    function (err) {
                        assert.ifError(err);
                        done();
                    },
            ...
            describe(vnodes ? "with vnodes" : "with single token", () => {
                ...
                describe("#getTokenRanges()", () => {
                    it(
                        ...
                        (done) => {
                            const ranges = Array.from(
                                client.metadata.getTokenRanges(),
                            );
                            assert.strictEqual(
                                ranges.length,
                                expectedTokenRanges,
                            ...
                            const replicas = client.metadata.getReplicas(
                                "ks_simple_rf1",
                                keyBuf,
                            );
                            const replicasByToken = client.metadata.getReplicas(
                                "ks_simple_rf1",
                                keyToken,
                            );
                            ...
                            utils.timesLimit(
                                ...
                                (n, timesNext) => {
                                    utils.eachSeries(
                                        ...
                                        (range, sNext) => {
                                            client.execute(
                                                "SELECT i from ks_simple_rf1.foo where token(i) > ? and token(i) <= ?",
                                                [
                                                        ...
                                                        (row) => {
                                                            if (row.i === key) {
                                                                if (
                                                                    ...
                                                                    assert.fail(
                                                                        "Found the same key in two ranges: " +
                                                                            foundRange +
                                                                            " and " +
                                                                            range,
                                                                    );
                    ...
                    it("should only unwrap at most one range for all ranges", () => {
                        ...
                        let wrappedRanges = ranges.filter((range) =>
                            range.isWrappedAround(),
                        );
                        assert.ok(
                            wrappedRanges.length <= 1,
                            "Should have been at most one wrapped range, but found: " +
                                wrappedRanges,
                        );
                        ...
                        wrappedRanges = splitRanges.filter((range) =>
                            range.isWrappedAround(),
                        );
                        assert.ok(
                            wrappedRanges.length <= 1,

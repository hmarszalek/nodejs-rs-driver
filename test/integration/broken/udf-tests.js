...
vdescribe("2.2", "Metadata @SERVER_API", function () {
    ...
    before(
        helper.ccmHelper.start(1, {
            yaml: ["enable_user_defined_functions:true"],
        }),
    );
                            ...
                            function (client, eachNext) {
                                client.metadata.getFunction(
                                    ...
                                    function (err, func) {
                                        ...
                                        assert.strictEqual(
                                            func.body,
                                            'return Integer.toString(i) + "hello";',
                                        );
                                        eachNext();
                                    },
                                );
        ...
        describe("with no callback specified", function () {
            it("should return function in a promise", function () {
                ...
                return client
                    .connect()
                    .then(function () {
                        return client.metadata.getFunction(keyspace, "plus", [
                            "int",
                            "int",
                        ]);
                    })
                    ...
                    function checkMeta(next) {
                        client.metadata.getAggregates(
                            ...
                            function (err, aggregatesArray) {
                                ...
                                assert.strictEqual(
                                    aggregatesArray[0].name,
                                    "sum",
                                );
                                assert.strictEqual(
                                    aggregatesArray[1].name,
                                    "sum",
                                );
                                next();
                            },
                    ...
                    function checkMeta(next) {
                        client.metadata.getAggregate(
                            ...
                            function (err, aggregate) {
                                ...
                                assert.strictEqual(
                                    aggregate.keyspaceName,
                                    keyspace,
                                );
                                assert.strictEqual(
                                    aggregate.argumentTypes.length,
                                    1,
                                );
                                assert.strictEqual(
                                    aggregate.argumentTypes[0].code,
                                    types.dataTypes.int,
                                );
                                assert.strictEqual(
                                    aggregate.returnType.code,
                                    types.dataTypes.int,
                                );
                                assert.strictEqual(
                                    aggregate.stateFunction,
                                    "plus",
                                );
                                assert.strictEqual(
        ...
        describe("with no callback specified", function () {
            it("should return aggregate in a promise", function () {
                ...
                return client
                    .connect()
                    .then(function () {
                        return client.metadata.getAggregate(keyspace, "sum", [
                            "int",
                        ]);
                    })

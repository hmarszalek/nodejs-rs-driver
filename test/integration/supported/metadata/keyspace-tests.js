...
describe("Metadata#getKeyspace()", function () {
    ...
    describe("when called with keyspace name", function () {
            ...
            utils.series(
                [
                    ...
                    function verifyKeyspace(next) {
                        ...
                        assert.notProperty(ks.strategy, "replicationFactor");
            ...
            utils.series(
                [
                    ...
                    function verifyKeyspace(next) {
                        ...
                        assert.notProperty(ks.strategy, "datacenterRepfactors");
        ...
        it("should return the same objects across repeated getKeyspace() calls", function (done) {
            // getKeyspace() hands back the native metadata object itself, which is cached, so
            // repeated lookups of one keyspace yield the very same instance - not a fresh
            ...
            utils.series(
                [
                    ...
                    function verifyIdentity(next) {
                        ...
                        // The same keyspace object, not two wrappers over one native object.
                        assert.strictEqual(ks1, ks2);
                        // ...and the same underlying table/column objects.
                        assert.strictEqual(
        ...
        it("should return the same strategy object across repeated reads", function (done) {
            // The strategy is built once per keyspace and pinned, like the tables/views/udts
            ...
            utils.series(
                [
                    ...
                    function verifyStrategyIdentity(next) {
                        const ks = setupInfo.client.metadata.getKeyspace(
                            "test_ks_strategy_id",
                        );

                        assert.strictEqual(ks.strategy, ks.strategy);
                        assert.strictEqual(
                            ks.strategy,
                            setupInfo.client.metadata.getKeyspace(
                                "test_ks_strategy_id",
...
describe("Metadata#getKeyspaces()", function () {
        ...
        utils.series(
            [
                ...
                function verifyKeyspaces(next) {
                    ...
                    assert.isAbove(Object.keys(keyspaces).length, 2);

                    const ks1 = keyspaces["test_ksall_1"];
                    ...
                    const ks2 = keyspaces["test_ksall_2"];
    ...
    it("should return the same underlying table object as getKeyspace() for the same keyspace", function (done) {
        // Metadata#getKeyspaces() and Metadata#getKeyspace() serve the same cached
        ...
        utils.series(
            [
                ...
                function verifyIdentity(next) {
                    const fromAll =
                        setupInfo.client.metadata.getKeyspaces()[
                            "test_ksall_identity"
                        ];
                    ...
                    // The same JS wrapper instance.
                    assert.strictEqual(fromAll, fromSingle);
    ...
    it("should return the same record on every call", function (done) {
        // The record is built once per cluster state and handed back as is, so callers can
        // compare it by identity rather than rebuilding a lookup structure of their own.
        const metadata = setupInfo.client.metadata;
        const first = metadata.getKeyspaces();

        assert.strictEqual(metadata.getKeyspaces(), first);
        done();
    });

    it("should hold the same keyspace objects as getKeyspace(), in either call order", function (done) {
        utils.series(
            [
                function recordFirst(next) {
                    const metadata = setupInfo.client.metadata;
                    const record = metadata.getKeyspaces();

                    Object.keys(record).forEach((name) => {
                        assert.strictEqual(
                            record[name],
                            metadata.getKeyspace(name),
                            `${name} differs between the record and getKeyspace()`,
                        );
                    });
                    next();
                },
                ...
                function singleFirst(next) {
                    const metadata = setupInfo.client.metadata;
                    const single = metadata.getKeyspace("test_ksall_order");

                    assert.strictEqual(
                        metadata.getKeyspaces()["test_ksall_order"],
                        single,
                    );
                    next();
                },

    ...
    describe("#execute(query, params, {prepare: 1}, callback)", function () {
            ...
            it("should callback with error when the partition key can not be encoded", (done) => {
                utils.eachSeries(
                    [
                        ...
                        [{}, types.Long.ONE],
                        [
                            types.InetAddress.fromString("10.10.10.10"),
                            types.Long.ONE,
                        ],
                    ],
                    (params, next) =>
        ...
        it("should serialize all guessed types", function (done) {
            const values = [
                ...
                "111",
                null,
                new types.Long(0x1001, 0x0109aa),
                1,
                utils.allocBufferFromArray([1, 240]),
                true,
                new Date(1221111111),
                types.InetAddress.fromString("10.12.0.1"),
                null,
                null,
                null,
            ];
        ...
        it("should serialize all null values", function (done) {
            ...
            const columnNames =
                "id, ascii_sample, text_sample, int_sample, bigint_sample, double_sample, blob_sample, boolean_sample, timestamp_sample, inet_sample, timeuuid_sample, list_sample, set_sample";
            serializationTest(setupInfo.client, values, columnNames, done);
        });
        it("should use prepared metadata to determine the type of params in query", function (done) {
            const values = [
                ...
                -100019,
                ["arr"],
                types.InetAddress.fromString("192.168.1.1"),
            ];
            const columnNames =
                "id, timeuuid_sample, list_sample2, map_sample, int_sample, float_sample, set_sample, inet_sample";
            serializationTest(setupInfo.client, values, columnNames, done);
        });
            ...
            utils.series(
                [
                    ...
                    function selectData(seriesNext) {
                        client.execute(
                            util.format(
                                "SELECT id, val, varintAsBlob(val) FROM %s",
                                table,
                            ),
                            [],
                            { prepare: 1 },
        ...
        it("should encode and decode decimal values", function (done) {
            ...
            utils.series(
                [
                    ...
                    function selectData(seriesNext) {
                        client.execute(
                            util.format("SELECT id, val FROM %s", table),
                            [],
                            { prepare: 1 },
                            function (err, result) {
                                assert.ifError(err);
        ...
        describe("with named parameters", function () {
            ...
            vit(
                "2.0",
                "should allow associative array of parameters",
                function (done) {
                    const query = util.format(
                        "SELECT * FROM %s WHERE id1 = :id1",
                        commonTable,
            ...
            vit(
                "2.0",
                "should allow objects with other props as parameters",
                function (done) {
                    const query = util.format(
                        "SELECT * FROM %s WHERE id1 = :ID1",
                        commonTable,
        ...
        it("should encode and decode maps using Map polyfills", function (done) {
            ...
            const values = [
                [
                    ...
                    new MapPF([
                        [-100, new Date(1423499543481)],
                        [1, new Date()],
                    ]),
                    new MapPF([
                        [new Date(1413496543466), -2],
                        [new Date(), 1.1233799457550049],
                    ]),
            ...
            const createTableCql = util.format(
                "CREATE TABLE %s " +
                    ...
                    "map_timeuuid_text map<timeuuid,decimal>)",
                table,
            );
            utils.series(
                [
                    ...
                    function selectData(seriesNext) {
                        ...
                        client.execute(
                            ...
                            function (err, result) {
                                ...
                                result.rows.forEach(function (row) {
                                    const expectedValues = helper.first(
                                        values,
                                        (item) => item[0].equals(row.id),
                                    );
                                    helper.assertInstanceOf(
                                        row["map_text_text"],
                                        MapPF,
                                    ...
                                    assert.strictEqual(
                                        row["map_text_text"].toString(),
                                        expectedValues[1].toString(),
                                    );
                                    assert.strictEqual(
                                        row["map_int_date"].toString(),
                                        expectedValues[2].toString(),
                                    );
                                    assert.strictEqual(
                                        row["map_date_float"].toString(),
                                        expectedValues[3].toString(),
                                    );
                                    assert.strictEqual(
                                        row["map_varint_boolean"].toString(),
                                        expectedValues[4].toString(),
                                    );
                                    assert.strictEqual(
                                        row["map_timeuuid_text"].toString(),
                                        expectedValues[5].toString(),
                                    );
                                });
                                seriesNext();
                            },
                        ...
                        client.execute(
                            ...
                            function (err, result) {
                                ...
                                result.rows.forEach(function (row) {
                                    ...
                                    assert.strictEqual(
                                        row["set_text"].toString(),
                                        expectedValues[1].toString(),
                                    );
                                    assert.strictEqual(
                                        row["set_timestamp"].toString(),
                                        expectedValues[2].toString(),
                                    );
                                    assert.strictEqual(
                                        row["set_float"].toString(),
                                        expectedValues[3].toString(),
                                    );
                                    assert.strictEqual(
                                        row["set_bigint"].toString(),
                                        expectedValues[4].toString(),
                                    );
            ...
            utils.series(
                [
                    function insert(next) {
                        const query = util.format(
                            "INSERT INTO %s (id1, id2, text_sample) VALUES (?, ?, ?)",
                            commonTable,
                        );
                        const params = [
                            id,
                            types.TimeUuid.now(),
                            "hello sample timestamp",
                        ];
                        client.execute(
                            query,
                            params,
                            { timestamp: timestamp, prepare: 1 },
                            next,
                        );
                    },
                    function select(next) {
                        const query = util.format(
                            "SELECT text_sample, writetime(text_sample) from %s WHERE id1 = ?",
                            commonTable,
                        );
                        client.execute(
                            query,
                ...
                [
                    ...
                    function select(next) {
                        ...
                        client.execute(query, [id], function (err, result) {
                            ...
                            assert.strictEqual(
                                row["list1"][0][0].toString(),
                                list[0][0].toString(),
                            );
                            assert.ok(Array.isArray(row["list1"][1]));
                            assert.strictEqual(
                                row["list1"][1][0].toString(),
                                list[1][0].toString(),
                            );
                            assert.strictEqual(
                                row["list1"][1][1].toString(),
                                list[1][1].toString(),
                            );
                            next();
                        });
        ...
        /* vit(
            ...
            function (done) {
                ...
                const params = {
                    ...
                    sample: utils.stringRepeat("c", 6 * 1024),
                };
                client.execute(
                    query,
                    params,
            ...
            utils.series(
                [
                    ...
                    function select2(next) {
                        const query = util.format(
                            "SELECT id1, id2, text_sample, map_sample FROM %s WHERE id1 = ?",
                            commonTable,
                        );
                        client2.execute(
                            query,
            ...
            utils.series(
                [
                    function testListWithNull(next) {
                        const query = util.format(
                            "INSERT INTO %s (id1, id2, list_sample) VALUES (?, ?, ?)",
                            commonTable,
                        );
                        client.execute(
                            query,
                            [Uuid.random(), tid, [tid, null]],
                            { prepare: true },
                            function (err) {
                                helper.assertInstanceOf(err, TypeError);
                                next();
                    ...
                    function testListWithUnset(next) {
                        ...
                        client.execute(
                            query,
                            [Uuid.random(), tid, [tid, types.unset]],
                            { prepare: true },
                            function (err) {
                                helper.assertInstanceOf(err, TypeError);
                                next();
                    ...
                    function testSetWithNull(next) {
                        const query = util.format(
                            "INSERT INTO %s (id1, id2, set_sample) VALUES (?, ?, ?)",
                            commonTable,
                        );
                        client.execute(
                            query,
                            [Uuid.random(), tid, [1, null]],
                            { prepare: true },
                            function (err) {
                                helper.assertInstanceOf(err, TypeError);
                                next();
                    ...
                    function testMapWithNull(next) {
                        ...
                        const query = util.format(
                            "INSERT INTO %s (id1, id2, map_sample) VALUES (?, ?, ?)",
                            commonTable,
                        );
                        client.execute(
                            query,
            ...
            utils.series(
                [
                    function insertDataWithEmptyStringValues(next) {
                        const query = util.format(
                            "INSERT INTO %s (id, map_sample, list_sample, set_sample) VALUES (?, ?, ?, ?)",
                            commonTable2,
                        );
                        client.execute(
                            query,
        ...
        it("should callback with response error when partition key parameter is invalid", (done) => {
            const client = setupInfo.client;
            const query = util.format(
                "SELECT * FROM %s WHERE id2 = ? AND id1 = ?",
                commonTable,
            );
        ...
        /* describe("with a different keyspace", function () {
            it("should fill in the keyspace in the query options passed to the lbp", () => {
                ...
                return client
                    .connect()
                    .then(() =>
                        client.execute(query, [Uuid.random()], {
                            prepare: true,
                        }),
                    )
                    ...
                    .then(() => {
                        const options =
                            executionOptionsArray[
                                executionOptionsArray.length - 1
                            ];
        ...
        describe("with udt and tuple", function () {
            ...
            vit("2.1", "should encode objects into udt", function (done) {
                ...
                const selectQuery =
                    "SELECT id, phone_col, address_col FROM tbl_udts WHERE id = ?";
                const client = setupInfo.client;
                const id = Uuid.random();
                const phone = {
                    alias: "work2",
                        ...
                        function select(next) {
                            client.execute(
                                ...
                                function (err, result) {
                                    ...
                                    assert.strictEqual(
                                        addressResult.phones.length,
                                        1,
                                    );
                                    assert.strictEqual(
                                        addressResult.phones[0].alias,
                                        address.phones[0].alias,
                                    );
                                    assert.strictEqual(
                                        addressResult.phones[0].number,
                                        null,
                                    );
                                    next();
                                },
                    ...
                    [
                        ...
                        function insert2(next) {
                            client.execute(
                                ...
                                [
                                    Uuid.random(),
                                    new types.Tuple("unset pair", undefined),
                                    null,
                                ],
                                options,
                                next,
                        ...
                        function insert3(next) {
                            client.execute(
                                ...
                                [
                                    Uuid.random(),
                                    new types.Tuple("null pair", null),
                                    null,
                                ],
                                options,
                                next,
                        ...
                        function select1(next) {
                            client.execute(
                                ...
                                function (err, result) {
                                    ...
                                    assert.strictEqual(
                                        tuple2Result.get(0).toString(),
                                        tuple2.get(0).toString(),
                                    );
                                    assert.strictEqual(
                                        tuple2Result.get(1).toString(),
                                        "12",
                                    );
                                    assert.strictEqual(
                                        tuple2Result.get(2),
            ...
            vit(
                ...
                () => {
                    ...
                    return client
                        .execute(insertQuery, [id1, tuple1, tuple2], options)
                        ...
                        .then((rs) => {
                            ...
                            assert.strictEqual(tuple1Result.length, 2);
                            assert.deepStrictEqual(tuple1Result.values(), [
                                "value1",
                                undefined,
                            ]);
        ...
        describe("with smallint and tinyint types", function () {
            const insertQuery =
                "INSERT INTO tbl_smallints (id, smallint_sample, tinyint_sample) VALUES (?, ?, ?)";
            const selectQuery =
                "SELECT id, smallint_sample, tinyint_sample FROM tbl_smallints WHERE id = ?";
            before(function (done) {
                const query =
                    "CREATE TABLE tbl_smallints " +
        ...
        describe("with secondary indexes", function () {
            ...
            vit(
                "2.1",
                "should be able to retrieve using index on frozen list",
                function (done) {
                    const client = setupInfo.client;
                    const table = helper.getRandomName("tbl");
                    utils.series(
            ...
            vit(
                "2.1",
                "should be able to retrieve using index on map keys",
                function (done) {
                    const client = setupInfo.client;
                    const table = helper.getRandomName("tbl");
                    utils.series(
                        [
                            ...
                            function selectData(seriesNext) {
                                const query = util.format(
                                    "SELECT * FROM %s WHERE v CONTAINS KEY ?",
                                    table,
                                );
                                client.execute(
                                    query,
            ...
            vit(
                "2.1",
                "should be able to retrieve using index on map values",
                function (done) {
                    const client = setupInfo.client;
                    const table = helper.getRandomName("tbl");
                    utils.series(
                        [
                            ...
                            function selectData(seriesNext) {
                                const query = util.format(
                                    "SELECT * FROM %s WHERE v CONTAINS ?",
                                    table,
                                );
                                client.execute(
                                    query,
            ...
            vit(
                "2.2",
                "should be able to retrieve using index on map entries",
                function (done) {
                    const client = setupInfo.client;
                    const table = helper.getRandomName("tbl");
                    utils.series(
        ...
        vdescribe("3.0", "with materialized views", function () {
            ...
            before(function createTables(done) {
                ...
                utils.eachSeries(
                    queries,
                    setupInfo.client.execute.bind(setupInfo.client),
            ...
            it("should choose the correct coordinator based on the partition key", function (done) {
                ...
                utils.timesSeries(
                    ...
                    function (n, timesNext) {
                        ...
                        const query =
                            "SELECT * FROM alltimehigh WHERE game = ?";
                        ...
                        client.execute(
                            ...
                            function (err, result) {
                                assert.ifError(err);
                                const queriedHostLastOctet = helper.lastOctetOf(
                                    result.info.queriedHost,
                                );
                                assert.strictEqual(
                                    queriedHostLastOctet,
                                    replicaByKey.get(game),
                                );
                                timesNext();
                            },
                        );
                    ...
                    [
                            ...
                            client.execute(
                                ...
                                (err, result) => {
                                    ...
                                    assert.strictEqual(
                                        ...
                                        3,
                                    ); // columns k, a, and c should be present.
                        ...
                        (next) => {
                            ...
                            client.execute(
                                query,
                                null,
                                { prepare: true, pageState: pageState },
                                (err, result) => {
                                    assert.ok(result);
                                    assert.strictEqual(
                                        result.columns.length,
                                        4,
                                    ); // columns k, a, b, and c should be present.
                                    assert.strictEqual(result.rows.length, 5);
                                    // data should be present with expected values.
                                    result.rows.forEach((row, index) => {
                                        assert.strictEqual(row.k, 0);

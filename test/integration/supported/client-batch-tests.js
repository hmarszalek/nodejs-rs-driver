...
describe("Client @SERVER_API", function () {
    ...
    describe("#batch(queries, {prepare: 0}, callback)", function () {
        ...
        vit(
            "2.0",
            "should execute a batch of queries with no params",
            function (done) {
                const insertQuery =
                    "INSERT INTO %s (id, text_sample) VALUES (%s, '%s')";
        ...
        vit(
            "2.0",
            "should execute a batch of queries with params",
            function (done) {
                const insertQuery =
                    "INSERT INTO %s (id, double_sample) VALUES (?, ?)";
                const selectQuery = "SELECT * FROM %s WHERE id = %s";
                const id1 = types.Uuid.random();
                const id2 = types.Uuid.random();
                const client = newInstance();
                const queries = [
                    {
                        query: util.format(insertQuery, table1),
                        params: [id1, 1000],
                    },
                    {
                        query: util.format(insertQuery, table2),
                        params: [id2, 2000.2],
                    },
                ];
                utils.series(
        ...
        vit(
            "2.0",
            "should callback with error when there is a ResponseError",
            function (done) {
                const client = newInstance();
        ...
        vit(
            "2.0",
            "should fail if non-existent profile provided",
            function (done) {
                const client = newInstance();
                client.batch(
                    ["INSERT WILL FAIL"],
        ...
        vit("2.0", "should use hints when provided", function (done) {
            ...
            client.batch(
                queries,
                { hints: hints, consistency: types.consistencies.quorum },
                function (err) {
                    assert.ifError(err);
        ...
        vit("2.1", "should support protocol level timestamp", function (done) {
            ...
            const queries = [
                {
                    query: util.format(insertQuery, table1),
                    params: [id1, "value 1 with timestamp"],
                },
                {
                    query: util.format(insertQuery, table2),
        ...
        describe("with no callback specified", function () {
            vit(
                "2.0",
                "should return a promise when batch completes",
                function () {
                    const insertQuery =
                        "INSERT INTO %s (id, text_sample) VALUES (%s, '%s')";
                    const selectQuery = "SELECT * FROM %s WHERE id = %s";
    ...
    describe("#batch(queries, {prepare: 1}, callback)", function () {
        ...
        vit("2.0", "should prepare and send the request", function (done) {
            ...
            client.batch(
                queries,
                { prepare: true, consistency: consistency },
                function (err) {
                    assert.ifError(err);
        ...
        vit(
            ...
            function (done) {
                ...
                const queries = [
                    {
                        query: util.format(
                            "INSERT INTO %s (id, time, int_sample) VALUES (?, ?, ?)",
                            table1,
                        ),
                        params: [
                            types.Uuid.random(),
        ...
        vit(
            ...
            function (done) {
                ...
                const query1Table2 = util.format(
                    "INSERT INTO %s (id, time, timestamp_sample, int_sample) VALUES (?, ?, ?, 202)",
                    table2,
                );
                const query2Table1 = util.format(
                    "INSERT INTO %s (id, time, decimal_sample, int_sample) VALUES (?, ?, ?, 301)",
                    table1,
                );
                const query2Table2 = util.format(
                    "INSERT INTO %s (id, time, float_sample, int_sample) VALUES (?, ?, ?, 302)",
                    table2,
                );
                const client = newInstance();
                utils.parallel(
        ...
        vit("2.0", "should allow named parameters", function (done) {
            ...
            const queries = [
                {
                    query: util.format(
                        "INSERT INTO %s (id, time, text_sample) VALUES (:paramId, :time, :text_sample)",
                        table1,
                    ),
                    params: {

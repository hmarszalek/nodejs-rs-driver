...
context("with a reusable 3 node cluster", function () {
            ...
            utils.timesLimit(
                ...
                function (n, next) {
                    client.execute(
                        ...
                        function (err, result) {
                            assert.ifError(err);
                            const lastOctet = helper.lastOctetOf(
    ...
    vdescribe("2.0", "TokenAwarePolicy", function () {
        ...
        it("should target correct replica for composite routing key", function (done) {
            ...
            const query =
                "INSERT INTO table_composite (id1, id2) VALUES (?, ?)";
            const queryOptions = {
                traceQuery: true,
                prepare: true,
        ...
        /* it("should target the correct replica using routing indexes", function (done) {
            ...
            const query =
                "INSERT INTO table_composite (id2, id1) VALUES (?, ?)";
            const queryOptions = {
        ...
        it("should target the correct replica using user-provided Token routingKey", function (done) {
            testWithQueryOptions(
                (client) => {
                    ...
                    const ranges = client.metadata.getTokenRangesForHost(
                        "ks_network_rp1",
                        host,
                    );
            ...
            utils.timesLimit(
                ...
                function (n, timesNext) {
                    client.execute(
                        ...
                        function (err, result) {
                            ...
                            coordinators.add(
                                parseInt(
                                    helper.lastOctetOf(result.info.queriedHost),
                                    10,
                                ),
                            );
                            timesNext();
                ...
                function (err) {
                    ...
                    assert.deepStrictEqual(
                        Array.from(coordinators).sort(),
                        replicasByKey[`rf${rf}`].get(i).sort(),
                    );
                    nextParameters();
                },
            );

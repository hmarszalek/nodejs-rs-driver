...
describe("Client", function () {
                ...
                [
                    function createTable(next) {
                        client.eachRow(
                            ...
                            [],
                            noop,
                            helper.waitSchema(client, next),
                        );
                    },
                    function insert(next) {
                        const query =
                            "INSERT INTO %s (id, text_sample) VALUES (%s, 'text%s')";
                        utils.timesSeries(
                            length,
    ...
    describe("#eachRow(query, params, {prepare: 1})", function () {
        ...
        vit(
            "2.0",
            "should autoPage on parallel different tables",
            function (done) {
                const keyspace = helper.getRandomName("ks");
                ...
                utils.series(
                    [
                        function createKs(next) {
                            client.eachRow(
                                helper.createKeyspaceCql(keyspace, 3),
                                [],
                                noop,
                                helper.waitSchema(client, next),
                            );
                ...
                [
                    ...
                    function selectDataRemaining(seriesNext) {
                        ...
                        client.eachRow(
                            util.format("SELECT * FROM %s", table),
                            [],
                            { prepare: 1, fetchSize: 70, pageState: pageState },
                            function (n, row) {
                                assert.ok(row);
                                counter++;
                            },
        ...
        /* vit(
            ...
            function (done) {
                const client = newInstance({
                    keyspace: setupInfo.client.keyspace,
                });
        ...
        if (!helper.isWin()) {
            vit(
                "2.0",
                "should retrieve large result sets in parallel",
                function (done) {
                    insertSelectTest(
                        setupInfo.keyspace + "." + table,
                        50000,
                        20,
                        50000,
                        { prepare: true },
            ...
            vit(
                "2.0",
                "should query multiple times in parallel with query tracing enabled",
                function (done) {
                    insertSelectTest(
                        setupInfo.keyspace + "." + table,
                        50000,

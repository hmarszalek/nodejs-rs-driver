...
describe("Metadata", function () {
    ...
    describe("#refreshKeyspace()", function () {
        it("should reject if have not connected yet", () => {
            ...
            return metadata.refreshKeyspace("ks1").catch(validateMetadataErr());
        });
    ...
    describe("#refreshKeyspaces()", function () {
        it("should reject if have not connected yet", () => {
            const metadata = new Metadata(clientOptions.defaultOptions(), null);
            return metadata.refreshKeyspaces().catch(validateMetadataErr());
        });
        ...
        it("should parse C*2 keyspace metadata for simple strategy", function (done) {
            ...
            metadata.refreshKeyspaces(function (err) {
                ...
                expectRanges(metadata, "ks1", h1, [
                    [3, 4],
                    [4, 5],
                    [5, 0],
                ]);
                expectRanges(metadata, "ks1", h2, [
                    [4, 5],
                    [5, 0],
                    [0, 1],
                ]);
                expectRanges(metadata, "ks1", h3, [
                    [5, 0],
                    [0, 1],
                    [1, 2],
                ]);
                expectRanges(metadata, "ks1", h4, [
                    [0, 1],
                    [1, 2],
                    [2, 3],
                ]);
                expectRanges(metadata, "ks1", h5, [
                    [1, 2],
                    [2, 3],
                    [3, 4],
                ]);
        ...
        it("should parse C*2 keyspace metadata for network strategy", function (done) {
            ...
            metadata.refreshKeyspaces(function (err) {
                ...
                // ]5, 2] (1/2 ring)
                expectRanges(metadata, "ks2", h3, [
                    [0, 1],
                    [1, 2],
                    [5, 0],
                ]);
                // all nodes except ]3, 4]
                expectRanges(metadata, "ks2", h4, [
                    [0, 1],
                    [1, 2],
                    [2, 3],
                    [4, 5],
        ...
        it("should parse C*3 keyspace metadata for simple strategy", function (done) {
            ...
            metadata.refreshKeyspaces(function (err) {
                ...
                assert.ok(ks.strategyOptions);
                assert.strictEqual(
                    ks.strategyOptions["replication_factor"],
                    "3",
                );
    ...
    describe("#getReplicas()", function () {
        ...
        it("should return depending on the dc rf with network topology", function (done) {
            ...
            metadata.refreshKeyspaces(function () {
                const replicas = metadata.getReplicas(
                    "dummy",
                    utils.allocBufferFromArray([0]),
                );
                assert.ok(replicas);
        ...
        it("should return depending on the dc rf and rack with network topology", function (done) {
            ...
            metadata.refreshKeyspaces(function () {
                ...
                const replicasByToken = metadata.getReplicas("dummy", t);
                assert.deepEqual(replicasByToken, replicas);
                assert.deepEqual(replicas.map(getAddress), [
                    "0",
                    "1",
                    "2",
                    "3",
        ...
        it("should return quickly with many replicas and not enough nodes in a DC to satisfy RF.", function (done) {
            ...
            metadata.refreshKeyspaces(function () {
                ...
                replicas = metadata.getReplicas(
                    "dummy",
                    utils.allocBufferFromArray([51752]),
                );
                assert.ok(replicas);
                assert.deepEqual(replicas.map(getAddress), [
                    "40",
    ...
    describe("#getPreparedInfo()", function () {
        ...
        it("should remove excess prepared queries when _maxPrepared has been exceeded", function () {
            const maxPrepared = 3;
            const options = Object.assign(clientOptions.defaultOptions(), {
                maxPrepared,
            });
            const metadata = new Metadata(options, null);
            const loggedMessages = [];
            metadata.log = (level, message) =>
            ...
            assert.lengthOf(
                Array.from(preparedQueries._mapByKey.keys()),
                maxPrepared,
            );
            assert.lengthOf(
                Array.from(preparedQueries._mapById.keys()),
                maxPrepared,
            );
            assert.lengthOf(
                loggedMessages.filter(
                    ({ level, message }) =>
                        level === "warning" &&
                        message.indexOf("exceeded maximum") >= 0,
                ),
                1,
            );
        });
    ...
    describe("#getUdt()", function () {
        ...
        it("should reject if have not connected yet", () => {
            const metadata = new Metadata(clientOptions.defaultOptions(), null);
            return metadata.getUdt("ks1", "udt1").catch(validateMetadataErr());
        });
    ...
    describe("#getTrace()", function () {
        it("should reject if have not connected yet", () => {
            const metadata = new Metadata(clientOptions.defaultOptions(), null);
            return metadata
                .getTrace(types.Uuid.random(), types.consistencies.all)
        ...
        it("should return the trace if its already stored", function (done) {
            ...
            metadata.getTrace(
                ...
                function (err, trace) {
                    ...
                    assert.strictEqual(
                        trace.events[0].activity,
                        eventRows[0].activity,
                    );
                    done();
                },
            );
        ...
        it("should parse the new client address column", function (done) {
            const sessionRow = {
                session_id: types.Uuid.fromString(
                    "69e37ff0-0475-11e5-9798-f3efee551757",
                ),
    ...
    describe("#getTable()", function () {
        ...
        it("should query each time if metadata retrieval flag is false", function (done) {
            const tableRow = {
                ...
                flags: ["compound"],
                gc_grace_seconds: 864000,
                id: "7e0e8bf0-5862-11e5-84f8-c7d0c38d1d8d",
            ...
            const columnRows = [
                ...
                },
                {
                    keyspace_name: "ks_tbl_meta",
                    table_name: "tbl1",
                    column_name: "text_sample",
                    clustering_order: "none",
                    column_name_bytes: "0x746578745f73616d706c65",
                    kind: "regular",
                    position: -1,
                    type: "text",
                },
                ...
                function (err, results) {
                    ...
                    assert.lengthOf(
                        cc.query
                            .getCalls()
                            .filter((call) =>
                                call.args[0].match(/system_schema.tables/),
                            ),
                        100,
                    );
        ...
        describe("with C*2.0 metadata rows", function () {
            it("should parse partition and clustering keys", function (done) {
                const customType =
                    "org.apache.cassandra.db.marshal.DynamicCompositeType(s=>org.apache.cassandra.db.marshal.UTF8Type, i=>org.apache.cassandra.db.marshal.Int32Type)";
                const tableRow = {
                    keyspace_name: "ks_tbl_meta",
                    columnfamily_name: "tbl1",
                    bloom_filter_fp_chance: 0.02,
            ...
            it("should parse table with compact storage with clustering key", function (done) {
                const tableRow = {
                    ...
                    bloom_filter_fp_chance: 0.01,
                    caching: '{"keys":"ALL", "rows_per_partition":"NONE"}',
                    cf_id: types.Uuid.fromString(
                        "96c86920-ed84-11e4-8991-199e66562428",
                    ),
                    column_aliases: '["id2"]',
                    comment: "",
                    compaction_strategy_class:
            ...
            it("should parse custom index (legacy)", function (done) {
                ...
                const columnRows = [
                    ...
                    },
                    {
                        ...
                        index_options:
                            '{"foo":"bar", "class_name":"dummy.DummyIndex"}',
                        index_type: "CUSTOM",
                        type: "regular",
                        validator: "org.apache.cassandra.db.marshal.UTF8Type",
                    },
                ...
                const metadata = new Metadata(
                    clientOptions.defaultOptions(),
                    getControlConnectionForTable(tableRow, columnRows),
                );
                metadata.initialized = true;
                metadata.keyspaces = { ks_tbl_meta: { tables: {} } };
                metadata.getTable("ks_tbl_meta", "tbl1", function (err, table) {
            ...
            it("should parse keys index with null string index options", function (done) {
                ...
                const columnRows = [
                    ...
                    },
                    {
                        ...
                        component_index: null,
                        index_name: "cfs_archive_parent_path",
                        index_options: '"null"',
                        index_type: "KEYS",
                        type: "regular",
                        validator: "org.apache.cassandra.db.marshal.UTF8Type",
        ...
        describe("with C*1.2 metadata rows", function () {
            it("should parse partition and clustering keys", function (done) {
                ...
                const metadata = new Metadata(
                    clientOptions.defaultOptions(),
                    getControlConnectionForTable(tableRow, columnRows, null, 1),
                );
                metadata.initialized = true;
                metadata.keyspaces = { ks_tbl_meta: { tables: {} } };
                metadata.getTable("ks_tbl_meta", "tbl1", function (err, table) {
                    ...
                    assert.strictEqual(
                        table.columnsByName["valcus3"].type.code,
                        dataTypes.custom,
                    );
                    assert.strictEqual(
                        table.columnsByName["valcus3"].type.info,
                        customType,
                    );
        ...
        describe("with C*2.2 metadata rows", function () {
            it("should parse new 2.2 types", function (done) {
                ...
                const tableRow = {
                    keyspace_name: "ks_tbl_meta",
                    columnfamily_name: "tbl_c22",
                    bloom_filter_fp_chance: 0.03,
                    caching: '{"keys":"ALL", "rows_per_partition":"NONE"}',
                    cf_id: types.Uuid.fromString(
                        "c05f4c40-fe05-11e4-8481-277ff03b5030",
                    ),
                    comment: "",
                    compaction_strategy_class:
                        "org.apache.cassandra.db.compaction.SizeTieredCompactionStrategy",
                ...
                const columnRows = [
                    ...
                    },
                    {
                        keyspace_name: "ks_tbl_meta",
                        columnfamily_name: "tbl_c22",
                        column_name: "smallint_sample",
                        component_index: 0,
                        index_name: null,
                        index_options: "null",
                        index_type: null,
                        type: "regular",
                        validator: "org.apache.cassandra.db.marshal.ShortType",
                    },
                    {
                        keyspace_name: "ks_tbl_meta",
                        columnfamily_name: "tbl_c22",
                        column_name: "time_sample",
                        component_index: 0,
                        index_name: null,
                        index_options: "null",
                        index_type: null,
                        type: "regular",
                        validator: "org.apache.cassandra.db.marshal.TimeType",
                    },
                    {
                        keyspace_name: "ks_tbl_meta",
                        columnfamily_name: "tbl_c22",
                        column_name: "tinyint_sample",
                        component_index: 0,
                        index_name: null,
                        index_options: "null",
                        index_type: null,
                        type: "regular",
                        validator: "org.apache.cassandra.db.marshal.ByteType",
                    },
                    {
                        keyspace_name: "ks_tbl_meta",
                        columnfamily_name: "tbl_c22",
                        column_name: "custom_sample",
                        component_index: 0,
                        index_name: null,
                        index_options: "null",
                        index_type: null,
        ...
        describe("with C*3.0+ metadata rows", function () {
            it("should parse partition and clustering keys", function (done) {
                ...
                const columnRows = [
                    ...
                    },
                    {
                        ...
                        column_name: "valz1",
                        clustering_order: "none",
                        column_name_bytes: "0x76616c7a31",
                        kind: "regular",
                        position: -1,
                        type: "int",
                    },
                    {
                        ...
                        column_name: "valcus3",
                        clustering_order: "none",
                        column_name_bytes: "0x76611663757333",
                        kind: "regular",
                        position: -1,
                        type: "'" + customType + "'",
                    },
                ...
                metadata.getTable("ks_tbl_meta", "tbl4", function (err, table) {
                    ...
                    assert.strictEqual(table.columns.length, 6);
                    assert.deepEqual(
                        table.columns.map((x) => x.type.code),
                        [
                            dataTypes.text,
                            dataTypes.uuid,
                            dataTypes.blob,
            ...
            it("should parse with compact storage", function (done) {
                ...
                const tableRow = {
                    ...
                    flags: ["dense"],
                    gc_grace_seconds: 864000,
                    id: "80fd9590-5862-11e5-84f8-c7d0c38d1d8d",
                    max_index_interval: 2048,
                    memtable_flush_period_in_ms: 0,
                    min_index_interval: 128,
                    read_repair_chance: 0,
                ...
                const columnRows = [
                    ...
                    },
                    {
                        ...
                        column_name: "text1",
                        clustering_order: "none",
                        column_name_bytes: "0x7465787431",
                        kind: "regular",
                        position: -1,
                        type: "text",
                    },
            ...
            it("should parse with compact storage with pk", function (done) {
                ...
                const tableRow = {
                    ...
                    flags: [],
                    gc_grace_seconds: 864000,
                    id: "81a32460-5862-11e5-b0ce-c7d0c38d1d8d",
                    max_index_interval: 2048,
                    memtable_flush_period_in_ms: 0,
                    min_index_interval: 128,
                    read_repair_chance: 0,
                ...
                const columnRows = [
                    {
                        ...
                        column_name: "column1",
                        clustering_order: "asc",
                        column_name_bytes: "0x636f6c756d6e31",
                        kind: "clustering",
                        position: 0,
                        type: "text",
                    },
                    ...
                    },
                    {
                        ...
                        column_name: "text2",
                        clustering_order: "none",
                        column_name_bytes: "0x7465787432",
                        kind: "static",
                        position: -1,
                        type: "text",
                    },
                    {
                        ...
                        column_name: "value",
                        clustering_order: "none",
                        column_name_bytes: "0x76616c7565",
                        kind: "regular",
                        position: -1,
                        type: "blob",
                    },
                ...
                metadata.getTable("ks_tbl_meta", "tbl6", function (err, table) {
                    ...
                    assert.strictEqual(table.columns.length, 3);
                    assert.strictEqual(
                        Object.keys(table.columnsByName).length,
                        3,
                    );
                    assert.strictEqual(table.partitionKeys.length, 1);
                    assert.strictEqual(table.clusteringKeys.length, 0);
            ...
            it("should parse with compact storage and all columns as clustering keys", function (done) {
                ...
                const tableRow = {
                    ...
                    default_time_to_live: 0,
                    extensions: {},
                    flags: ["compound", "dense"],
                    gc_grace_seconds: 864000,
                    id: "b4d56ea0-5881-11e5-8326-c7d0c38d1d8d",
                    max_index_interval: 2048,
                    memtable_flush_period_in_ms: 0,
                    min_index_interval: 128,
        ...
        describe("with Thrift secondary index", function () {
            it("should parse tables from Thrift with secondary indexes", function (done) {
                const tableRow = {
                    keyspace_name: "ks1",
                    columnfamily_name: "ThriftSecondaryIndexTest",
                    bloom_filter_fp_chance: null,
                    caching: { keys: "ALL", rows_per_partition: "NONE" },
                    cf_id: "121d4950-41fd-11e7-ac8d-e1535f74c6ee",
                    column_aliases: [],
                    comparator: "org.apache.cassandra.db.marshal.UTF8Type",
                    compaction_strategy_options: "{}",
                    compression_parameters:
    ...
    describe("#getFunctions()", function () {
        ...
        describe("with C* 2.2 metadata rows", function () {
                ...
                const rows = [
                    {
                        ...
                        language: "java",
                        return_type: "org.apache.cassandra.db.marshal.LongType",
                    },
                ];
            ...
            it("should parse function metadata with 2 parameters", function (done) {
                ...
                const metadata = new Metadata(
                    clientOptions.defaultOptions(),
                    getControlConnectionForRows(rows),
                );
                metadata.initialized = true;
                metadata.keyspaces["ks_udf"] = { functions: {} };
                metadata.getFunctions(
            ...
            it("should parse a function metadata with no parameters", function (done) {
                ...
                metadata.getFunctions(
                    ...
                    function (err, funcArray) {
                        ...
                        assert.strictEqual(
                            funcArray[0].argumentNames,
                            utils.emptyArray,
                        );
                        assert.strictEqual(
                            funcArray[0].argumentTypes.length,
                            0,
                        );
                        assert.strictEqual(funcArray[0].language, "java");
                        assert.ok(funcArray[0].returnType);
        ...
        it("should parse function metadata with 2 parameters", function (done) {
            const rows = [
                ...
                },
                {
                    ...
                    called_on_null_input: false,
                    language: "java",
                    return_type: "org.apache.cassandra.db.marshal.Int32Type",
                },
            ];
            const metadata = new Metadata(
                clientOptions.defaultOptions(),
                ...
                const rows = [
                    {
                        ...
                        final_func: null,
                        initcond: utils.allocBufferFromArray([
                            0, 0, 0, 0, 0, 0, 0, 0,
                        ]),
                        return_type: "org.apache.cassandra.db.marshal.LongType",
                        state_func: "plus",
                        state_type: "org.apache.cassandra.db.marshal.LongType",
                    },
                ];
                const cc = getControlConnectionForRows(rows);
                const metadata = new Metadata(
            ...
            it("should parse aggregate metadata with 1 parameter", function (done) {
                const rows = [
                    ...
                    },
                    {
                        ...
                        final_func: null,
                        initcond: utils.allocBufferFromArray([0, 0, 0, 0]),
                        return_type:
                            "org.apache.cassandra.db.marshal.Int32Type",
                        state_func: "plus",
                        state_type: "org.apache.cassandra.db.marshal.Int32Type",
                    },
                ];
                const metadata = new Metadata(
                    clientOptions.defaultOptions(),
                ...
                metadata.getAggregates(
                    "ks_udf1",
                    "sum",
                    function (err, aggregatesArray) {
                        assert.ifError(err);
                        assert.ok(aggregatesArray);
                        assert.strictEqual(aggregatesArray.length, 2);
                        assert.strictEqual(aggregatesArray[0].name, "sum");
                        assert.strictEqual(
                            aggregatesArray[0].keyspaceName,
                            "ks_udf1",
                        );
                        assert.strictEqual(
                            aggregatesArray[0].signature.join(", "),
                            ["bigint"].join(", "),
                        );
                        assert.strictEqual(
                            aggregatesArray[0].argumentTypes.length,
                            1,
                        );
                        assert.ok(aggregatesArray[0].argumentTypes[0]);
                        assert.strictEqual(
                            aggregatesArray[0].argumentTypes[0].code,
                            types.dataTypes.bigint,
                        );
                        assert.ok(aggregatesArray[0].returnType);
                        assert.strictEqual(
                            aggregatesArray[0].returnType.code,
                            types.dataTypes.bigint,
                        );
                        assert.strictEqual(
                            aggregatesArray[0].finalFunction,
                            null,
                        );
                        assert.ok(aggregatesArray[0].stateType);
                        assert.strictEqual(
                            aggregatesArray[0].stateType.code,
                            types.dataTypes.bigint,
                        );
                        assert.strictEqual(
                            aggregatesArray[0].stateFunction,
                            "plus",
                        );
                        assert.strictEqual(
                            aggregatesArray[0].initCondition,
                            "0",
                        );
                        ...
                        assert.strictEqual(
                            aggregatesArray[1].signature.join(", "),
                            ["int"].join(", "),
                        );
                        assert.strictEqual(
                            aggregatesArray[1].argumentTypes.length,
                            1,
                        );
                        assert.ok(aggregatesArray[1].argumentTypes[0]);
                        assert.strictEqual(
                            aggregatesArray[1].argumentTypes[0].code,
                            types.dataTypes.int,
                        );
                        assert.ok(aggregatesArray[1].returnType);
                        assert.strictEqual(
                            aggregatesArray[1].returnType.code,
                            types.dataTypes.int,
                        );
                        assert.strictEqual(
                            aggregatesArray[1].finalFunction,
                            null,
                        );
                        assert.ok(aggregatesArray[1].stateType);
                        assert.strictEqual(
                            aggregatesArray[1].stateType.code,
                            types.dataTypes.int,
                        );
                        assert.strictEqual(
                            aggregatesArray[1].stateFunction,
                            "plus",
                        );
                        assert.strictEqual(
                            aggregatesArray[1].initCondition,
                            "0",
                        );
                        done();
                    },
            ...
            it("should parse aggregate metadata with 1 parameter", function (done) {
                ...
                metadata.getAggregates(
                    ...
                    function (err, aggregatesArray) {
                        ...
                        assert.strictEqual(
                            typeof aggregatesArray[1].initCondition,
                            "string",
                        );
                        assert.strictEqual(
                            aggregatesArray[1].initCondition,
    ...
    describe("#getMaterializedView()", function () {
        ...
        it("should reject if have not connected yet", () => {
            const metadata = new Metadata(clientOptions.defaultOptions(), null);
            return metadata
                .getMaterializedView("ks_mv", "view1")
                .catch(validateMetadataErr());
        });
        ...
        it("should set sorted tokens from a single host", function (done) {
            ...
            const expectedRanges = new Set(
                [
                    ...
                    (r) => new TokenRange(token(r[0]), token(r[1]), tokenizer),
                ),
            );
            ...
            assert.deepEqual(
                metadata.getReplicas(null, Array.from(ranges)[0]),
                [h1],
            );
            done();
        });
    ...
    describe("#newToken()", function () {
        ...
        it("should create newToken from multiple Buffer components", function () {
            const token = metadata.newToken([
                utils.allocBufferFromArray([0, 0, 0, 1]),
                utils.allocBufferFromArray([0, 0, 0, 2]),
            ]);
            assert.strictEqual(
                token.getValue().toString(),
                "-5168409507470576865",
        ...
        it("should throw an error if tokenizer isn't set yet", function () {
            const metadataNoTokenizer = new Metadata(
                clientOptions.defaultOptions(),
                null,
            );
    ...
    describe("#newTokenRange()", function () {
        const metadata = new Metadata(clientOptions.defaultOptions(), null);
        metadata.tokenizer = new tokenizer.Murmur3Tokenizer();
        const token0 = metadata.newToken(
            utils.allocBufferFromArray([0, 0, 0, 1]),
        );
        const token1 = metadata.newToken(
            utils.allocBufferFromArray([0, 0, 0, 2]),
        );
    ...
    describe("#compareSchemaVersions()", function () {
        ...
        it("should return true when versions match", () => {
            const cc = {
                connection: {
                    send: () =>
                        Promise.resolve({ rows: [{ schema_version: "123" }] }),
                },
            };
        ...
        it("should return false when versions do not match", () => {
            ...
            const cc = {
                connection: {
                    send: () =>
                        Promise.resolve({
                            rows: [{ schema_version: String(index++) }],
                        }),
                },
            };
...
function getControlConnectionForTable(
    ...
    return getControlConnectionForResponse(
        (q) => {
            if (
                q.indexOf("system.schema_columnfamilies") >= 0 ||
                q.indexOf("system_schema.tables") >= 0
            ) {
                return { rows: [tableRow] };
            }
...
function getControlConnectionForResponse(
    responseOrHandler,
    err,
    protocolVersion = 4,
) {

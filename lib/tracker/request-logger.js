...
class RequestLogger extends RequestTracker {
    ...
    constructor(options) {
         ...
         * @type {Boolean}
         */
        this.logNormalRequests = this.#options.logNormalRequests;
         ...
         * @type {Boolean}
         */
        this.logErroredRequests = this.#options.logErroredRequests;
    ...
    onSuccess(
        ...
        if (
            this.#options.slowThreshold > 0 &&
            toMillis(latency) > this.#options.slowThreshold
        ) {
            this.#logSlow(
                host,
                query,
            ...
            this.#options.requestSizeThreshold > 0 &&
            requestLength > this.#options.requestSizeThreshold
        ) {
            this.#logLargeRequest(
                host,
                query,
                parameters,
                execOptions,
    ...
    #logSlow(
        ...
        const message = format(
            ...
            host.addressToString(),
            Math.floor(toMillis(latency)),
            getPayloadSizes(requestLength, responseLength),
            getStatementInfo(query, parameters, execOptions, this.#options),
        );
    ...
    #logLargeErrorRequest(
        ...
        const maxStackTraceLength =
            this.#options.messageMaxErrorStackTraceLength ||

...
class SslOptions {
    ...
    /**
     * Optionally override the trusted CA certificates. When not provided,
     * uses OpenSSL defaults (which in most cases means system trusted certificates).
     * Setting this option replaces the default CA certificates, rather than adding to them.
     *
     ...
     *
     * Corresponds to [SSL_CTX_use_certificate](https://docs.openssl.org/master/man3/SSL_CTX_use_certificate/)
...
function validateProtocolOptions(protocolOptions) {
    ...
    const schemaAgreementWait = protocolOptions.maxSchemaAgreementWaitSeconds;
    if (
        schemaAgreementWait !== undefined &&
        (!Number.isSafeInteger(schemaAgreementWait) ||
            schemaAgreementWait < 0 ||
            schemaAgreementWait > 0xffffffff)
    ) {
        throw new TypeError(
            "protocolOptions.maxSchemaAgreementWaitSeconds must be an integer between 0 and 4294967295",
        );
    }
    const autoAwaitSchema = protocolOptions.autoAwaitSchemaAgreement;
    if (autoAwaitSchema !== undefined && typeof autoAwaitSchema !== "boolean") {
        throw new TypeError(
...
function validateApplicationInfo(options) {
    ...
    switch (true) {
        case options.id === null:
        case options.id === undefined:
        case options.id instanceof types.Uuid:
        case typeof options.id === "string":
            break;
        default:
            throw new TypeError("Client id must be either a Uuid or a string");
    }
}
...
function normalizeSslOptions(sslOptions) {
    ...
    if (normalized.minVersion) {
        normalized.minVersion = convertTlsVersion(normalized.minVersion);
    }

    if (normalized.maxVersion) {
        normalized.maxVersion = convertTlsVersion(normalized.maxVersion);
    }
    ...
    // The options require that the number of keys and certs match
    ...
    // We can check only with cert, as we ensure cert is provided when and only when the key is provided.
    if (!!normalized.pfx && !!normalized.cert) {
        throw new errors.ArgumentError(
            "Only one of (pfx) or (cert with key) must be provided in sslOptions.",
        );
    }
...
function setRustOptions(options) {
    ...
    if (options.policies) {
        ...
        if (options.policies.retry) {
            try {
                rustOptions.retryPolicy =
                    options.policies.retry.getRustConfiguration();
            } catch (e) {
        ...
        if (options.policies.addressResolution) {
            switch (options.policies.addressResolution.constructor) {
                ...
                default:
                    throw new Error(
                        "Currently only MappingAddressTranslator is supported as address resolution policy",
                    );
            }
        }
    }
    ...
    if (options.protocolOptions) {
        if (options.protocolOptions.noCompact !== undefined) {
            ...
            throwNotSupported("Protocol option noCompact");
        }
        if (
    ...
    // Same motivation as for protocol option noCompact.
    if (options.cloud !== undefined) {
        throwNotSupported("Cloud options");
    }
    if (options.monitorReporting !== undefined) {
